...
class CrimeReportController {
    ...
    try {
      ...
      if (adminUser) {
        await pushNotification({
          ...
          content: `A new crime report has been submitted in ${province}.`,
          read: false,
        });
      }
      ...
      const policeNotifications = policeUsers.map((police) =>
        pushNotification({
          userId: police._id,
          title: "New Crime Report in Your Area",
          content: `A new crime has been reported in ${province}. Please review.`,
          read: false,
        })
      );

      await Promise.all(policeNotifications);
  ...
  static updateReportStatus = async (req, res) => {
    try {
      ...
      let notificationTitle = "Crime Report Update";
      let notificationContent = "";

      if (status === "in-progress") {
        notificationContent = "Your crime report is now under investigation.";
      } else if (status === "rejected") {
        notificationContent = "Your crime report has been rejected.";
      } else if (status === "resolved") {
        notificationContent = "Your crime report has been resolved successfully.";
      }
